import asyncio
import random
from datetime import datetime
from typing import List

from aiogram import Bot
from database.queries import get_active_chat_ids
from logger import logger
from config import REQUEST_DELAY

# Список юморных вечерних фраз
EVENING_HUMOR_PHRASES = [
    "🌆 Вот и день прошел! Кто-то булочка дня, а кто-то только мечтает, как бы ее съесть... 🥐😋",
    "🌇 Вечерок подкрался! Сегодняшняя булочка дня уже остыла, но завтра появится новая горячая возможность! 🔥🥖",
    "🌅 Закат булочного дня! Одни считают очки, другие считают калории от съеденных воображаемых булочек... 📊🍩",
    "⭐ Звезды зажигаются, а наши пекари подводят итоги! Интересно, снятся ли булочкам дня люди? 🌟👥",
    "🌃 Тишина вечера нарушается только хрустом виртуальных круассанов в мечтах игроков... 🥐💭",
    "🌙 Луна взошла, пекарня закрывается, а булочники уходят спать с мыслями о завтрашнем тесте! 🌛⚡",
    "🌆 День окончен, счетчик очков обнулен в сердцах, но надежда на булочное величие вечна! 💫❤️",
    "🎭 Занавес опускается на сегодняшний спектакль 'Кто кого перебулочит'! Завтра — новые роли! 🎪🎨",
    "🌇 Вечерняя сводка: кто-то стал теплым хлебушком, кто-то остался черствой корочкой... До завтра! 📰🍞",
    "🌅 Солнце село, но дух соперничества в пекарне никогда не спит! Сладких булочных снов! 😴🥐✨",
]


async def send_evening_humor(bot: Bot):
    """Отправка вечернего юморного сообщения во все активные чаты."""
    try:
        chat_ids = await get_active_chat_ids()
        if not chat_ids:
            logger.warning("Нет активных чатов для отправки вечерних сообщений.")
            return

        # Выбираем случайную фразу
        humor_message = random.choice(EVENING_HUMOR_PHRASES)

        sent_count = 0
        error_count = 0

        for chat_id in chat_ids:
            try:
                await bot.send_message(chat_id=chat_id, text=humor_message)
                sent_count += 1
                logger.info(f"Вечернее юморное сообщение отправлено в чат {chat_id}")

                # Задержка между отправками для избежания flood control
                await asyncio.sleep(REQUEST_DELAY)

            except Exception as e:
                error_count += 1
                logger.error(
                    f"Ошибка при отправке вечернего сообщения в чат {chat_id}: {e}"
                )

        logger.info(
            f"Вечерние сообщения отправлены: успешно {sent_count}, ошибок {error_count}"
        )

    except Exception as e:
        logger.error(f"Ошибка при отправке вечерних юморных сообщений: {e}")


def get_random_evening_phrase() -> str:
    """Получить случайную вечернюю фразу."""
    return random.choice(EVENING_HUMOR_PHRASES)


def get_moscow_hour() -> int:
    """Получить текущий час по московскому времени (UTC+3)."""
    from datetime import timezone, timedelta

    moscow_tz = timezone(timedelta(hours=3))
    moscow_time = datetime.now(moscow_tz)
    return moscow_time.hour


def is_evening_time() -> bool:
    """Проверить, подходящее ли время для вечерних сообщений (18:00-22:00 МСК)."""
    hour = get_moscow_hour()
    return 18 <= hour <= 22


def get_random_evening_cron() -> str:
    """Получить случайное время в формате cron для вечерних сообщений (18:00-22:00 МСК)."""
    from datetime import timezone, timedelta, datetime

    # Получаем текущее московское время
    moscow_tz = timezone(timedelta(hours=3))
    moscow_now = datetime.now(moscow_tz)
    current_hour = moscow_now.hour
    current_minute = moscow_now.minute

    # Определяем диапазон времени для планирования
    if current_hour < 18:
        # Еще рано для вечерних сообщений - планируем на сегодня
        min_hour = 18
        max_hour = 22
        target_day = "сегодня"
    elif current_hour <= 22:
        # Сейчас вечернее время - планируем оставшееся время сегодня или завтра
        if current_hour == 22 and current_minute > 30:
            # Слишком поздно для сегодня, планируем на завтра
            min_hour = 18
            max_hour = 22
            target_day = "завтра"
        else:
            # Можем еще запланировать на сегодня, но только на будущее время
            min_hour = current_hour if current_minute < 30 else current_hour + 1
            max_hour = 22
            target_day = "сегодня"

            # Если остается мало времени, планируем на завтра
            if min_hour > max_hour:
                min_hour = 18
                max_hour = 22
                target_day = "завтра"
    else:
        # Поздно - планируем на завтра
        min_hour = 18
        max_hour = 22
        target_day = "завтра"

    # Случайный час в диапазоне
    hour = random.randint(min_hour, max_hour)

    # Случайная минута от 0 до 59, но если это текущий час - то минимум на 10 минут вперед
    if target_day == "сегодня" and hour == current_hour:
        min_minute = current_minute + 10
        if min_minute >= 60:
            # Переходим на следующий час
            hour = min(hour + 1, 22)
            min_minute = 0
        minute = random.randint(min_minute, 59)
    else:
        minute = random.randint(0, 59)

    # aiocron работает по московскому времени, конвертация не нужна
    # Формат: минута час * * * (каждый день в указанное время)
    cron_schedule = f"{minute} {hour} * * *"

    # Логируем информацию о планировании
    logger.warning(
        f"Запланировано вечернее сообщение на {hour:02d}:{minute:02d} МСК ({target_day}) - cron: {cron_schedule}"
    )

    return cron_schedule


def get_evening_schedule_info() -> dict:
    """Получить информацию о текущем расписании вечерних сообщений."""
    moscow_hour = get_moscow_hour()
    from datetime import timezone, timedelta

    moscow_tz = timezone(timedelta(hours=3))
    moscow_time = datetime.now(moscow_tz)

    return {
        "current_moscow_time": moscow_time.strftime("%H:%M:%S"),
        "current_moscow_hour": moscow_hour,
        "is_evening_time": is_evening_time(),
        "evening_window": "18:00-22:00 МСК",
        "next_possible_time": "Сегодня" if moscow_hour < 22 else "Завтра",
    }
