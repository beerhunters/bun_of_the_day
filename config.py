...
try:
    ...
    LOG_TO_TELEGRAM = os.environ.get("LOG_TO_TELEGRAM", "true").lower() in [
        "true",
        "1",
        "yes",
    ]
    ...
    )  # Задержка между запросами в секундах (1 сообщение в секунду для безопасности)
    MAX_RETRIES = int(
        os.environ.get("MAX_RETRIES", "3")
    )  # Максимальное количество попыток
    RETRY_DELAY = int(
        os.environ.get("RETRY_DELAY", "5")
    )  # Базовая задержка для повторных попыток

    # Проверяем валидность уровней для файла и телеграма
    if LOG_FILE_LEVEL not in valid_levels:
        print(
            f"Warning: Invalid LOG_FILE_LEVEL '{LOG_FILE_LEVEL}', using 'ERROR' instead"
        )
    ...
    # Настройки умной обработки timeout ошибок
    TIMEOUT_ALERT_MINUTES = int(os.getenv("TIMEOUT_ALERT_MINUTES", "10"))
    TIMEOUT_REMINDER_HOURS = int(os.getenv("TIMEOUT_REMINDER_HOURS", "1"))

except KeyError as ex:
    print(f"Error: Missing required environment variable: {ex}")
    print(
        "Please ensure the following variables are set: API_TOKEN, ADMIN_ID, FOR_LOGS"
    )
    ...
    print(
        "  LOG_FILE_LEVEL (DEBUG, INFO, WARNING, ERROR, CRITICAL) - File logging level"
    )
    print(
        "  LOG_TELEGRAM_LEVEL (DEBUG, INFO, WARNING, ERROR, CRITICAL) - Telegram notifications level"
    )
    sys.exit(1)
except ValueError as ex:
    print(f"Error: Invalid value for environment variable: {ex}")
    print(
        "Please check that ADMIN_ID and FOR_LOGS are valid integers and API_TOKEN is not empty"
    )
    sys.exit(1)
